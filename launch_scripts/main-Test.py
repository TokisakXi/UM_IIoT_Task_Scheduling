...
# episode.run()
# print(
#     f"随机调度\t环境时间：{episode.env.now} {time.time() - tic} {average_completion(episode)} {average_slowdown(episode)}")
...
episode = Episode(machine_configs, jobs_configs, algorithm, event_file)
episode.run()
print(f"首适应算法\t环境时间：{episode.env.now} {time.time() - tic} {average_completion(episode)} {average_slowdown(episode)}")
...
# episode.run()
# print(f"Tetris调度\t环境时间：{episode.env.now} {time.time() - tic} {average_completion(episode)} {average_slowdown(episode)}")
